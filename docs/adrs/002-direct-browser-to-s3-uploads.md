...
Expose a `POST /upload-url` route that returns a presigned S3 `PUT` URL. The browser uploads the MP3 directly to the private uploads bucket with XHR progress events, then calls `POST /transcribe` with the resulting S3 key.
